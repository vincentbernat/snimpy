...
class TestMibSnimpy(unittest.TestCase):
    def setUp(self):
        ...
        self.nodes = ["snimpy", "snimpyScalars", "snimpyTables", "snimpyTraps"]
        self.nodes.sort()
        self.tables = [
            "snimpyComplexTable",
    ...
    def testGetNodes(self):
        ...
        self.assertEqual(self.nodes, snodes)
        for n in nodes:
    ...
    def testGet(self):
        ...
        for i in self.notifications:
            self.assertEqual(str(mib.get("SNIMPY-MIB", i)), i)
            self.assert_(
                isinstance(mib.get("SNIMPY-MIB", i), mib.Notification)
            )
    ...
    def testGetByOid(self):
        ...
        for i in self.notifications:
            ...
            self.assertEqual(str(mib.getByOid(nodebyname.oid)), i)
            self.assert_(
                isinstance(mib.getByOid(nodebyname.oid), mib.Notification)
            )
    ...
    def testGetType(self):
        ...
        self.assertEqual(
            b"PhysAddress", mib.ffi.string(mib._getType("PhysAddress").name)
        )
        self.assertEqual(
            b"InetAddress", mib.ffi.string(mib._getType(b"InetAddress").name)
        )
    ...
    def testEnums(self):
        ...
        self.assertEqual(
            ...
            {0: "first", 1: "second", 2: "third", 7: "last", 8: "secondByte"},
        )
    ...
    def testImplied(self):
        ...
        self.assertEqual(
            mib.get("SNIMPY-MIB", "snimpySimpleTable").implied, False
        )
        self.assertEqual(
            mib.get("SNIMPY-MIB", "snimpyComplexTable").implied, False
        )
        self.assertEqual(
            mib.get("SNIMPY-MIB", "snimpyIndexTable").implied, True
        )

    def testOid(self):
        """Test that objects are rooted at the correct OID"""
        oids = {
            "snimpy": (1, 3, 6, 1, 2, 1, 45121),
    ...
    def testTypeOverrides_Errors(self):
        ...
        # Unknown type.
        self.assertRaises(
            mib.SMIException, setattr, attr, "typeName", "SomeUnknownType.kjgf"
        )

        # Incompatible basetype.
        self.assertRaises(
            mib.SMIException, setattr, attr, "typeName", "InetAddressType"
        )
