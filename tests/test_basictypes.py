...
class TestBasicTypes(unittest.TestCase):
    ...
    def testStringEncoding(self):
        ...
        self.assertEqual(a, "hello")
        a = basictypes.build(
            "SNIMPY-MIB", "snimpyUnicodeString", u"\U0001F60E Hello"
        )
        self.assertEqual(a, u"\U0001F60E Hello")
        a = basictypes.build(
            "SNIMPY-MIB", "snimpyUnicodeString", b"\xf0\x9f\x98\x8e Hello"
        )
        self.assertEqual(a, u"\U0001F60E Hello")
        self.assertRaises(
            UnicodeError,
    ...
    def testIpAddress(self):
        ...
        a = basictypes.build(
            "SNIMPY-MIB", "snimpyIpAddress", socket.inet_aton("10.0.4.5")
        )
    ...
    def testEnum(self):
        ...
        self.assertRaises(
            ValueError, basictypes.build, "SNIMPY-MIB", "snimpyEnum", "unknown"
        )
        self.assertEqual(str(a), "down(2)")
    ...
    def testOid(self):
        ...
        # Suboid
        self.assert_(
            (list(mib.get("SNIMPY-MIB", "snimpyInteger").oid) + [2, 3]) in a
        )
        self.assert_(
            (list(mib.get("SNIMPY-MIB", "snimpyInteger").oid)[:-1] + [29, 3])
            not in a
        )
    ...
    def testTimeticks(self):
        ...
        self.assertEqual(
            a,
            basictypes.build("SNIMPY-MIB", "snimpyTimeticks", timedelta(1, 3)),
        )
        self.assert_(
            a
            < basictypes.build(
                "SNIMPY-MIB", "snimpyTimeticks", timedelta(100, 30)
            )
        )
    ...
    def testBits(self):
        ...
        a = basictypes.build(
            "SNIMPY-MIB", "snimpyBits", set(["first", "second"])
        )
        self.assertEqual(a, [0, 1])
    ...
    def testStringAsBits(self):
        ...
        a = basictypes.build(
            "SNIMPY-MIB", "snimpyOctetString", b"\x17\x00\x01"
        )
        self.assert_(isinstance(a, basictypes.OctetString))
        b = [7, 6, 5, 3, 23]
        for i in range(30):
    ...
    def testPacking(self):
        ...
        self.assertEqual(
            basictypes.build(
                "SNIMPY-MIB", "snimpyString", "Hello world"
            ).pack(),
        ...
        self.assertEqual(
            basictypes.build(
                "SNIMPY-MIB", "snimpyIpAddress", "10.11.12.13"
            ).pack(),
            rfc1902.IpAddress("10.11.12.13"),
        )
        self.assertEqual(
            basictypes.build(
                "SNIMPY-MIB", "snimpyObjectId", (1, 2, 3, 4)
            ).pack(),
            rfc1902.univ.ObjectIdentifier((1, 2, 3, 4)),
        )
        self.assertEqual(
            basictypes.build(
                "SNIMPY-MIB", "snimpyTimeticks", timedelta(3, 2)
            ).pack(),
    ...
    def testOidConversion(self):
        ...
        tt = {
            ...
            ("snimpyIndexVarLen", "hello1", False): tuple(
                [len("hello1")] + [ord(a) for a in "hello1"]
            ),
            ("snimpyIndexFixedLen", "hello2", False): tuple(
                ord(a) for a in "hello2"
            ),
            ("snimpyIndexImplied", "hello3", True): tuple(
                ord(a) for a in "hello3"
            ),
            ("snimpyIndexImplied", "hello3", False): tuple(
                [len("hello3")] + [ord(a) for a in "hello3"]
            ),
        }
        for key in tt:
            t, v, implied = key
            oid = basictypes.build("SNIMPY-MIB", t, v).toOid(implied)
            self.assertEqual(oid, tt[key])
    ...
    def testOidGreedy(self):
        ...
        tt = {
            ...
            ("snimpyIndexImplied", False): (
                (6, 104, 101, 108, 108, 111, 50),
                (7, "hello2"),
            ),
            ("snimpyComplexFirstIP", False): (
                (15, 15, 16, 100, 23, 74, 87),
                (4, "15.15.16.100"),
            ),
            ("snimpySimpleIndex", False): ((17, 19, 20), (1, 17)),
            ("snimpyIndexOidVarLen", False): (
                (3, 247, 145, 475568, 475, 263),
                (4, (247, 145, 475568)),
            ),
        }
        for key in tt:
        ...
        # Test if too short
        tt = {
            "snimpyComplexFirstIP": ((17, 19, 20), False),
    ...
    def testDisplay(self):
        ...
        self.assertEqual(
            str(basictypes.build("SNIMPY-MIB", "snimpyInteger", 18)), "0.18"
        )
        self.assertEqual(
            str(basictypes.build("SNIMPY-MIB", "snimpyInteger", 8)), "0.08"
        )
        self.assertEqual(
            str(basictypes.build("SNIMPY-MIB", "snimpyInteger", 288)), "2.88"
        )
        self.assertEqual(
            str(basictypes.build("SNIMPY-MIB", "snimpyInteger", 28801)),
            "288.01",
        )
        self.assertEqual(
            str(basictypes.build("SNIMPY-MIB", "snimpyString", "test")), "test"
        )
        self.assertEqual(
            str(basictypes.build("SNIMPY-MIB", "snimpyOctetString", b"test")),
            str(b"test"),
        )
        self.assertEqual(
            str(
                basictypes.build("SNIMPY-MIB", "snimpyOctetString", b"tes\x05")
            ),
            str(b"tes\x05"),
        )
    ...
    def testDisplayFormat(self):
        ...
        with mock.patch(
            "snimpy.mib.Node.fmt", new_callable=mock.PropertyMock
        ) as e:
            e.return_value = "255a"
            ...
            a = basictypes.build(
                "SNIMPY-MIB", "snimpyString", b"\x03testtest..."
            )
            self.assertEqual(str(a), "te:st:te+st...")

            e.return_value = "2a1x:"
            a = basictypes.build("SNIMPY-MIB", "snimpyString", b"aatest")
    ...
    def testInputFormat(self):
        ...
        with mock.patch(
            ...
            e.return_value = "*2a:+255a"
            a = basictypes.build(
                "SNIMPY-MIB", "snimpyString", u"te:st:te+st..."
            )
            self.assertEqual(a.pack(), b"\x03testtest...")

            e.return_value = "2a1x:"
            a = basictypes.build(
                "SNIMPY-MIB", "snimpyString", u"aa74:65:73:74"
            )
            ...
            e.return_value = "3a"
            a = basictypes.build("SNIMPY-MIB", "snimpyString", u"a\n\r")
            self.assertEqual(a.pack(), b"a\n\r")

    def testRepr(self):
        """Test representation"""
        self.assertEqual(
            repr(basictypes.build("SNIMPY-MIB", "snimpyInteger", 18)),
            "<Integer: 0.18>",
        )
        self.assertEqual(
            repr(
        ...
        self.assertEqual(
            repr(
                basictypes.build(
                    "SNIMPY-MIB", "snimpyIpAddress", "124.24.14.3"
                )
            ),
            "<IpAddress: 124.24.14.3>",
        )
        self.assertEqual(
            repr(basictypes.build("SNIMPY-MIB", "snimpyString", "45754dfgf")),
            "<String: 45754dfgf>",
        )
        self.assertEqual(
            repr(basictypes.build("SNIMPY-MIB", "snimpyEnum", 2)),
            "<Enum: down(2)>",
        )
        self.assertEqual(
            repr(basictypes.build("SNIMPY-MIB", "snimpyBoolean", False)),
            "<Boolean: false(2)>",
        )
        self.assertEqual(
            repr(basictypes.build("SNIMPY-MIB", "snimpyCounter", 4547)),
            "<Unsigned32: 4547>",
        )
        self.assertEqual(
            repr(
                basictypes.build(
                    "SNIMPY-MIB", "snimpyBits", ["first", "second"]
                )
            ),
    ...
    def testLikeAString(self):
        ...
        a = basictypes.build("SNIMPY-MIB", "snimpyString", "4521dgf")
